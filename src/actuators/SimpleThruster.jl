...
get_actuator_force(A::SimpleThruster) = A.command.current_value ? A.joint_transform * A.force : SVector{6,Float64}(0.,0.,0.,0.,0.,0.)

function get_actuator_force!(A::SimpleThruster)
    A.current_force = A.command.current_value * A.joint_transform * A.force # : SVector{6,Float64}(0.,0.,0.,0.,0.,0.)
    return nothing
end